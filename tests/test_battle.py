...
def test_shadow_multipliers_apply_to_attack_and_defense():
    ...
    assert effective_defense(shadow) == pytest.approx(effective_defense(non_shadow) * 0.8333)
...
def test_stab_applies_when_move_type_matches_attacker_type():
    ...
    damage_without_stab = calculate_damage(persian_no_stab, defender, persian_no_stab.fast_move)
...
def test_lucario_vs_shadow_persian_slot_1():
    ...
    assert [m.move_id for m in result.player.completed_moves] == ["COUNTER_FAST", "COUNTER_FAST"]
...
def test_machamp_vs_shadow_persian_slot_1():
    ...
    assert [m.move_id for m in result.player.completed_moves] == ["BULLET_PUNCH_FAST"] * 2
    assert result.player.total_damage_dealt == 16
    assert result.player.final_energy == 22
    assert result.player.turns_used == 4  # 1 turn wasted -- a 3rd Bullet Punch doesn't fit
...
def test_player_uses_charge_move_against_slot_3():
    ...
    assert [m.move_id for m in result.player.completed_moves] == ["LOCK_ON_FAST"] * 4 + ["RETURN"]
...
def test_opponent_uses_charge_move_regardless_of_slot():
    player = shadow_persian()  # stand-in "attacker" role; is_shadow irrelevant to this check
    opponent = non_shadow_persian(fast_move=LOCK_ON_FAST, charge_move=RETURN)
    opponent.is_shadow = True

    result = simulate_turns(player, opponent, opponent_slot=1)

    assert [m.move_id for m in result.opponent.completed_moves] == ["LOCK_ON_FAST"] * 4 + ["RETURN"]
