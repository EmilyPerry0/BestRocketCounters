...
SCRATCH_FAST = Move("SCRATCH_FAST", "NORMAL", power=6.0, duration_turns=1, energy_delta=4)
FOUL_PLAY = Move("FOUL_PLAY", "DARK", power=70.0, duration_turns=4, energy_delta=-50)

LOW_KICK_FAST = Move("LOW_KICK_FAST", "FIGHTING", power=5.0, duration_turns=1, energy_delta=5)
EARTHQUAKE = Move("EARTHQUAKE", "GROUND", power=140.0, duration_turns=7, energy_delta=-100)

COUNTER_FAST = Move("COUNTER_FAST", "FIGHTING", power=13.0, duration_turns=2, energy_delta=9)
AURA_SPHERE = Move("AURA_SPHERE", "FIGHTING", power=100.0, duration_turns=4, energy_delta=-50)

MUD_SHOT_FAST = Move("MUD_SHOT_FAST", "GROUND", power=4.0, duration_turns=1, energy_delta=6)

BULLET_PUNCH_FAST = Move("BULLET_PUNCH_FAST", "STEEL", power=10.0, duration_turns=2, energy_delta=11)
CLOSE_COMBAT = Move("CLOSE_COMBAT", "FIGHTING", power=105.0, duration_turns=5, energy_delta=-100)
...
LOCK_ON_FAST = Move("LOCK_ON_FAST", "NORMAL", power=2.0, duration_turns=1, energy_delta=10)
...
def shadow_persian(fast_move: Move = SCRATCH_FAST, charge_move: Move = FOUL_PLAY) -> Pokemon:
    return Pokemon(
        species="PERSIAN", level=20, types=["NORMAL"],
        base_attack=150, base_defense=136, base_stamina=163,
        fast_move=fast_move, charge_move=charge_move, is_shadow=True,
    )


def shadow_kangaskhan() -> Pokemon:
    return Pokemon(
        species="KANGASKHAN", level=20, types=["NORMAL"],
        base_attack=181, base_defense=165, base_stamina=233,
        fast_move=LOW_KICK_FAST, charge_move=EARTHQUAKE, is_shadow=True,
    )
...
def non_shadow_persian(fast_move: Move = LOCK_ON_FAST, charge_move: Move = RETURN) -> Pokemon:
    """Used only for the charge-move mechanic tests (8/9), not as a Rocket opponent."""
    return Pokemon(
        species="PERSIAN", level=20, types=["NORMAL"],
        base_attack=150, base_defense=136, base_stamina=163,
        fast_move=fast_move, charge_move=charge_move, is_shadow=False,
    )


def player_lucario() -> Pokemon:
    return Pokemon(
        species="LUCARIO", level=40, types=["FIGHTING", "STEEL"],
        base_attack=236, base_defense=144, base_stamina=172,
        fast_move=COUNTER_FAST, charge_move=AURA_SPHERE, is_shadow=False,
    )


def player_excadrill() -> Pokemon:
    return Pokemon(
        species="EXCADRILL", level=25, types=["GROUND", "STEEL"],
        base_attack=255, base_defense=129, base_stamina=242,
        fast_move=MUD_SHOT_FAST, charge_move=EARTHQUAKE, is_shadow=False,
    )


def player_machamp() -> Pokemon:
    return Pokemon(
        species="MACHAMP", level=15, types=["FIGHTING"],
        base_attack=234, base_defense=159, base_stamina=207,
        fast_move=BULLET_PUNCH_FAST, charge_move=CLOSE_COMBAT, is_shadow=False,
    )
